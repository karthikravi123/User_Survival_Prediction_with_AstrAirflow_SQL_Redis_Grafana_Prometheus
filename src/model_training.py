...
class ModelTraining:
    ...
    def load_data_from_redis(self,entity_ids):
        try:
            logger.info("Extracing data from Redis")
    ...
    def prepare_data(self):
        try:
            ...
            logger.exception(f"error while preparing data {e}")
            raise CustomException(str(e))
    ...
    def hyperparameter_tunning(self,X_train,y_train):
        try:
            ...
            logger.info(f"Best parameter : {random_search.best_params_}")
            return random_search.best_estimator_
        
        except Exception as e:
            logger.exception(f"error while Hyperparameter tuning data {e}")
            raise CustomException(str(e))
    ...
    def train_and_evaluate(self,X_train,y_train,X_test,y_test):
        try:
            ...
            logger.exception(f"error while model training {e}")
            raise CustomException(str(e))
    ...
    def save_model(self,model):
        try:
            ...
            logger.exception(f"error while model saving {e}")
            raise CustomException(str(e))
        
    def run(self):
        try:
            logger.info("staring model training pipeline")
            X_train,X_test,y_train,y_test =self.prepare_data()
            accuracy = self.train_and_evaluate(X_train,y_train,X_test,y_test)
            logger.info("End of model training pipeline")

        except Exception as e:
            logger.exception(f"error while model training pipeline {e}")
            raise CustomException(str(e))
